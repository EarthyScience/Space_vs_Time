...
for step in range(1, 7):
    ...
    train_feat_split = train_feat[selected_indices].reshape(
        [num_sites_in_split * 8, 11, 64, 730]
    )
    train_stat_split = train_stat[selected_indices].reshape([num_sites_in_split * 8, 3])
    train_tar_split = train_tar[selected_indices].reshape(
        [num_sites_in_split * 8, 365, 2]
    )
...
for step in range(1, 7):
    ...
    print(selected_indices)
    # Extract the corresponding data
    val_feat_split = val_feat[selected_indices].reshape(
        [num_sites_in_split * 8, 11, 64, 730]
    )
...
for step in range(1, 7):
    ...
    if step == 1:
        ...
        train_feat_split = train_feat_split[:, selected_years].reshape(
            [num_sites_in_split * 14, 11, 64, 730]
        )
        train_stat_split = train_stat_split[:, selected_years].reshape(
            [num_sites_in_split * 14, 3]
        )
        train_tar_split = train_tar_split[:, selected_years].reshape(
            [num_sites_in_split * 14, 365, 2]
        )
    else:
        train_feat_split = train_feat_split[:, -num_years_in_split:].reshape(
            [num_sites_in_split * num_years_in_split, 11, 64, 730]
        )
        train_stat_split = train_stat_split[:, -num_years_in_split:].reshape(
            [num_sites_in_split * num_years_in_split, 3]
        )
        train_tar_split = train_tar_split[:, -num_years_in_split:].reshape(
            [num_sites_in_split * num_years_in_split, 365, 2]
        )

    print(train_feat_split.shape)
    # Save each split
    np.save(
        f"{save_path_timespace_set}/timespace_{step}_train_feat.npy", train_feat_split
    )
    np.save(
        f"{save_path_timespace_set}/timespace_{step}_train_stat.npy", train_stat_split
    )
    np.save(
        f"{save_path_timespace_set}/timespace_{step}_train_tar.npy", train_tar_split
    )
...
for step in range(1, 7):
    ...
    val_feat_split = val_feat_split[:, selected_years].reshape(
        [num_sites_in_split * num_years_in_split, 11, 64, 730]
    )
    val_stat_split = val_stat_split[:, selected_years].reshape(
        [num_sites_in_split * num_years_in_split, 3]
    )
    val_tar_split = val_tar_split[:, selected_years].reshape(
        [num_sites_in_split * num_years_in_split, 365, 2]
    )
