...
fluxnet_jsbach_pixel = pd.read_csv(
    pixel_dir / "fluxnetsite_to_jsbach_pixel.csv", index_col=0
)

# Drop specific invalid / unwanted sites (e.g., Italian sites)
fluxnet_jsbach_pixel = fluxnet_jsbach_pixel.drop([10, 12]).reset_index(drop=True)
...
for site in sites_of_interest:
    ...
    index_pixel = pixels[
        (pixels["Latitude"] == lat) & (pixels["Longitude"] == lon)
    ].index
...
train_sites = fluxnet_jsbach_pixel[fluxnet_jsbach_pixel["dset_type"] == "train"][
    "site_id"
]
...
for site in train_sites:
    ...
    if site == "CA-Oas":
        ...
        pixels["distance"] = np.sqrt(
            (pixels["Latitude"] - lat) ** 2 + (pixels["Longitude"] - lon) ** 2
        )
...
spaceset_filtered = pixels_spaceset[
    (pixels_spaceset["obs_set"] == False) & (pixels_spaceset["dset_type"].isna())
]
...
spacetimeset_filtered = pixels_spacetimeset[
    (pixels_spacetimeset["obs_set"] == False)
    & (pixels_spacetimeset["dset_type"].isna())
]
