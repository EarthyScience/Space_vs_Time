...
for lat_idx, lat in tqdm(enumerate(valid_pixels_mask.lat.values), total=len(valid_pixels_mask.lat.values), desc="Processing latitudes"):
    for lon_idx, lon in enumerate(valid_pixels_mask.lon.values):
        ...
        if valid_pixels_mask_values[lat_idx, lon_idx]:
            ...
            mean_tree_frac_pix = tree_frac_eu.treeFrac.sel(lat=lat, lon=lon).mean(dim="year").values
            pixel_info.append({"Latitude": lat, "Longitude": lon, "region": "EU", "mean_TreeFrac": mean_tree_frac_pix})
...
for lat_idx, lat in tqdm(enumerate(valid_pixels_mask.lat.values), total=len(valid_pixels_mask.lat.values), desc="Processing latitudes"):
    for lon_idx, lon in enumerate(valid_pixels_mask.lon.values):
        ...
        if valid_pixels_mask_values[lat_idx, lon_idx]:
            # Append the coordinates to the list
            mean_tree_frac_pix = tree_frac_na.treeFrac.sel(lat=lat, lon=lon).mean(dim="year").values
            pixel_info.append({"Latitude": lat, "Longitude": lon, "region": "NA", "mean_TreeFrac": mean_tree_frac_pix})
