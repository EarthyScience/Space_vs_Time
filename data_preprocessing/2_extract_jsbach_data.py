...
meteo_input = xr.open_mfdataset(
    JSBACH_DATA / "*.nc", drop_variables=["lat_bnds", "time_bnds"]
)
...
tar_nep = tar_nep.where(
    ~((tar_nep.time.dt.month == 2) & (tar_nep.time.dt.day == 29)), drop=True
)

tar_hfls = tar_hfls.where(
    ~((tar_hfls.time.dt.month == 2) & (tar_hfls.time.dt.day == 29)), drop=True
)
...
def extract_data(dataset, lat, lon):
    dataset = dataset.where(
        ~((dataset.time.dt.month == 2) & (dataset.time.dt.day == 29)), drop=True
    )
...
def calc_vpd(rh_data, ta_data):
    ...
    e_s = 611.21 * np.exp(
        (18.678 - ta_data_c / 234.5) * (ta_data_c / (257.14 + ta_data_c))
    )
...
for i in range(pixels.shape[0]):
    ...
    # load target data
    nep_values = tar_nep.nep.sel(
        lat=lat, lon=lon, time=slice("1902-01-01", "2021-12-31")
    ).values
    data_tar[:, :, 0] = np.reshape(nep_values, (120, 365))

    hfls_values = tar_hfls.hfls.sel(
        lat=lat, lon=lon, time=slice("1902-01-01", "2021-12-31")
    ).values
...
stat_feat_norm = min_max_norm2(
    stat_feat,
    np.array([mat_max, map_max, co2_max]),
    np.array([mat_min, map_min, co2_min]),
)
