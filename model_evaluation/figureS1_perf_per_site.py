...
time_r2iav_2 = pd.read_csv(f"{path}/{model_set}_r2iav_2_site.csv", index_col=0, header=0)
...
space_r2iav_2 = pd.read_csv(f"{path}/{model_set}_r2iav_2_site.csv", index_col=0, header=0)
...
timespace_r2iav = pd.read_csv(f"{path}/{model_set}_r2iav_site.csv", index_col=0, header=0)
timespace_r2iav_2 = pd.read_csv(f"{path}/{model_set}_r2iav_2_site.csv", index_col=0, header=0)
timespace_r2anom =pd.read_csv(f"{path}/{model_set}_r2anom_site.csv", index_col=0, header=0)
...
ax.scatter(timespace_r2_site_mean, timespace_r2_iav_mean, label="Time+space", color="#00b1ea")
...
fig = plt.figure(figsize=(width_in, height_in))
gs = GridSpec(3, 3, height_ratios=[1, 4, 0.2], width_ratios=[0.2, 4, 1], hspace=0.11, wspace=0.11)

# Main scatter plot (center)
ax_main = fig.add_subplot(gs[1, 1])
ax_main.scatter(space_r2_site_mean, space_r2_iav_mean, label="Space", color="#c6d325", alpha=0.7)
ax_main.scatter(time_r2_site_mean, time_r2_iav_mean, label="Time", color="#ef7c00", alpha=0.7)
ax_main.scatter(timespace_r2_site_mean, timespace_r2_iav_mean, label="Time+space", color="#00b1ea", alpha=0.7)
...
sns.kdeplot(x=timespace_r2_site_mean, ax=ax_top, color="#00b1ea", alpha=0.6, label="Time+space")
