...
anom_ig_space_mean = {
    p: np.load(f"{path}/anom_{p}_ig_space_mean_{num_years}.npy") for p in perc
}
anom_ig_time_mean = {
    p: np.load(f"{path}/anom_{p}_ig_time_mean_{num_years}.npy") for p in perc
}
...
def calc_ig_sum_year(ig_data):
    ...
    ig_anom_sum_year = np.sum(
        ig_data,
        where=np.logical_or(ig_data < loweser_thres, ig_data > upper_thres),
        axis=(2, 3),
    )
...
ax[0, 0].plot(
    x_vals, slope_space * x_vals + intercept_space, color="#c6d325", linestyle="--"
)
# Plot the regression line for the time model
ax[0, 0].plot(
    x_vals, slope_time * x_vals + intercept_time, color="#ef7c00", linestyle="--"
)

# Plot the regression line for the space model
ax[0, 1].plot(
    x_vals_high,
    slope_space_high * x_vals_high + intercept_space_high,
    color="#c6d325",
    linestyle="--",
)
# Plot the regression line for the time model
ax[0, 1].plot(
    x_vals_high,
    slope_time_high * x_vals_high + intercept_time_high,
    color="#ef7c00",
    linestyle="--",
)
...
ig_energy_space_low = (
    space_sum_year_low[:, 0] + space_sum_year_low[:, 6] + space_sum_year_low[:, 7]
)
ig_energy_time_low = (
    time_sum_year_low[:, 0] + time_sum_year_low[:, 6] + time_sum_year_low[:, 7]
)
ig_energy_diff_low = ig_energy_space_low - ig_energy_time_low

ig_water_space_low = (
    space_sum_year_low[:, 1]
    + space_sum_year_low[:, 2]
    + space_sum_year_low[:, 4]
    + space_sum_year_low[:, 5]
)
ig_water_time_low = (
    time_sum_year_low[:, 1]
    + time_sum_year_low[:, 2]
...
ig_energy_space_high = (
    space_sum_year_high[:, 0] + space_sum_year_high[:, 6] + space_sum_year_high[:, 7]
)
ig_energy_time_high = (
    time_sum_year_high[:, 0] + time_sum_year_high[:, 6] + time_sum_year_high[:, 7]
)
ig_energy_diff_high = ig_energy_space_high - ig_energy_time_high

ig_water_space_high = (
    space_sum_year_high[:, 1]
    + space_sum_year_high[:, 2]
    + space_sum_year_high[:, 4]
    + space_sum_year_high[:, 5]
)
ig_water_time_high = (
    time_sum_year_high[:, 1]
    + time_sum_year_high[:, 2]
    + time_sum_year_high[:, 4]
    + time_sum_year_high[:, 5]
)
...
for ax, label in zip(axes, labels):
    ax.text(
        -0.1,
        1.03,  # Position slightly outside the top-left corner
        label,
...
ax[0].plot(
    x_vals,
    slope_space * x_vals + intercept_space,
    color="#c6d325",
    linestyle="--",
    label="Space Regression",
)
# Plot the regression line for the time model
ax[0].plot(
    x_vals,
    slope_time * x_vals + intercept_time,
    color="#ef7c00",
    linestyle="--",
...
# Plot the 1:1 line (identity line)
ax[0].plot(
    [-6.1e-6, 6e-6], [-6.1e-6, 6e-6], color="black", linestyle="--", linewidth=1.5
)
